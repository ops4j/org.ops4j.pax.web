    ...
    implements HttpServiceListener
{
    ...
    private void registerHttpContext()
    {
            ...
            && m_httpService != null )
        {
            ...
            if( WebContainerUtils.isWebContainer( m_httpService ) )
            {
                ...
                {
                    contextparams.put( WebContainerConstants.CONTEXT_NAME, m_httpContextMapping.getPath() );
                }
                ...
                ( (WebContainer) m_httpService ).setContextParam(
                    DictionaryUtils.adapt( contextparams ),
                    m_httpContext
                );
            }
            registerWebElements();
        ...
        try
        {
            if( httpService != m_httpService )
            {
                throw new IllegalStateException(
                    "Unavailable http service [" + httpService
                    + "] is not equal with prior available http service [" + m_httpService + "]"
                );
            }
            unregisterWebElements();
            m_httpService = null;
    ...
    public String toString()
    {
        return new StringBuffer()
            ...
            .append( "{" )
            .append( "mapping=" ).append( m_httpContextMapping )
            .append( "}" )
            .toString();
    }
