...
public class DigesterFactory {
	...
	static {
		...
		// from JavaEE 7
		add(systemIds, XmlIdentifiers.WEB_31_XSD,
				locationFor("web-app_3_1.xsd"));
		add(systemIds, XmlIdentifiers.WEB_FRAGMENT_31_XSD,
				locationFor("web-fragment_3_1.xsd"));
	...
	private static String locationFor(String name) {
		...
		if (location == null) {
			...
			location = bundle.getResource("resources/" + name);
		}
		if (location == null) {
			Bundle bundle = FrameworkUtil.getBundle(DigesterFactory.class);
			location = bundle.getResource("javax/servlet/resources/" + name);
		}
		if (location == null) {
			Bundle bundle = FrameworkUtil.getBundle(DigesterFactory.class);
