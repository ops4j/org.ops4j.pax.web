...
public class TldScanner implements ServletContainerInitializer {
    ...
    // Logger
    private static Logger log = LoggerFactory.getLogger(TldScanner.class);
...
//    }
    ...
    public String[] getLocation(String uri) throws JasperException {
    	log.debug("looking for location for uri[{}]", uri);
        ...
        String[] returnMapping = mappings.get(uri);
        log.debug("returning the following: "+ returnMapping);
    ...
    private void scanTlds() throws JasperException {
    	log.debug("Scaning Tlds");
        mappings = new HashMap<String, String[]>();

        // Make a local copy of the system jar cache 
        jarTldCacheLocal.putAll(jarTldCache);
        boolean localTldsProcessed = true;

        try {
        	log.debug("process web.xml");
            processWebDotXml();
            
            log.debug("process jars");
            scanJars();

            // PAXWEB-86: Add support for searching TLDs also in the imported
            // packages
            log.debug("scan bundles in class space");
            scanBundlesInClassSpace();
            
            if (!localTldsProcessed) {
				log.debug("process tlds in Fielsystem");
	 ...
	 * 
	 * Asks the JasperClassLoader to scan all OSGi bundles in the Class Space
	 * (Imported + Required bundles) and handles all matches with the same
	 * behaviour as the original scanJar method
	 */
	...
	private void scanBundlesInClassSpace() throws JasperException, IOException {
		...
		if (webappLoader instanceof JasperClassLoader) {
			...
			for (URL url : urls) {
				...
				String uri = null;
				try {
                    TldInfo scanTld = scanTld(path, null, stream);
                    log.debug("found TldInfo {}", scanTld);
					uri = scanTld.getUri();
//					uri = getUriFromTld(path, stream);
					...
					if (stream != null) {
						try {
							...
							// do nothing
							log.info("trouble reading tld stuff");
						}
					}
				}
    ...
    private void processWebDotXml() throws Exception {
        ...
        log.debug("JspConfigDescriptor found {}", jspConfig);
        
        for (TaglibDescriptor taglib: jspConfig.getTaglibs()) {
        	log.debug("checking taglib of descriptor {}", taglib);
            if (taglib == null) {
                continue;
            }
            String tagUri = taglib.getTaglibURI();
            String tagLoc = taglib.getTaglibLocation();
            log.debug("checking tagUri: {} and tagLoc: {} of taglib", tagUri, tagLoc);
            ...
            if (systemUris.contains(tagUri)
            	...
            	log.debug("found a systmUris containing the tagUri, skipping it: {}", tagUri);
                continue;
            }
            ...
            log.debug( "Add tld map from web.xml: " + tagUri + "=>" + tagLoc+ "," + tagLoc2);
    ...
    private void addListener(TldInfo tldInfo, boolean isLocal) {
    	log.debug("add Listener called for tldInfo: {}", tldInfo);
        String uri = tldInfo.getUri();
        log.debug("Uri of TLd: {}", uri);
        if (!systemUrisJsf.contains(uri)
                    || (isLocal && useMyFaces)
                    || (!isLocal && !useMyFaces)) {
            for (String listenerClassName: tldInfo.getListeners()) {
                log.debug( "Add tld listener {}", listenerClassName);
    ...
    private void mapTldLocation(String resourcePath, TldInfo tldInfo,
                                boolean isLocal) {
    	log.debug("MapTLD locations for resource: {} and tldInfo: {}", resourcePath, tldInfo);
        String uri = tldInfo.getUri();
        if (uri == null) {
            return;
        }
        ...
        if ((isLocal
            ...
            log.debug("Add tld map from tld in " +
    ...
    private void scanJars() throws Exception {
        ...
        while (loader != null) {
            if (loader instanceof URLClassLoader) {
                ...
                for (int i=0; i<urls.length; i++) {
                	...
                	if ("link".equalsIgnoreCase(urls[i].getProtocol())) {
                		log.debug("skipped: {}", urls[i]);
                		continue;
                	}
        ...
        log.debug("TLdMapd is now: {}", tldMap);
        if (tldMap != null) {
