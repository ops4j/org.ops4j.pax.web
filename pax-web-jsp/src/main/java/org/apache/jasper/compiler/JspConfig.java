...
public class JspConfig {
    ...
    private void processWebDotXml() {
        ...
        if (jspConfig == null) {
            //parse the web.xml to get the jspConfig as a fallback
            try {
                parseWebDotXml(ctxt);
            } catch (JasperException e) {
                log.error("error when parse the web.xml", e);
            }
            return;
        }
    ...
    private void parseWebDotXml(ServletContext ctxt) throws JasperException {
        ...
        try {
            ...
            Element webApp = getRootElement(is);
            
                       
            if (webApp == null ||
                getAttribute(webApp, "version") == null ||
                Double.valueOf(getAttribute(webApp, "version")).doubleValue() < 2.4) {
                defaultIsELIgnored = "true";
                return;
            }

            Element jspConfig = getChild(webApp, "jsp-config");
            if (jspConfig == null) {
                return;
            }

            jspProperties = new Vector<JspPropertyGroup>();
            Element[] jspPropertyList = getChildren(jspConfig, "jsp-property-group");
            for (Element element : jspPropertyList) {
            
                Element[] list = getChildren(element);
                ...
                for (Element innerElement : list) {
                    ...
                    if ("url-pattern".equals(tname))
                        urlPatterns.addElement(getTextContent(innerElement));
                    else if ("page-encoding".equals(tname))
                        pageEncoding = getTextContent(innerElement);
                    else if ("is-xml".equals(tname))
                        isXml = getTextContent(innerElement);
                    else if ("el-ignored".equals(tname))
                        elIgnored = getTextContent(innerElement);
                    else if ("scripting-invalid".equals(tname))
                        scriptingInvalid = getTextContent(innerElement);
                    else if ("include-prelude".equals(tname))
                        includePrelude.addElement(getTextContent(innerElement));
                    else if ("include-coda".equals(tname))
                        includeCoda.addElement(getTextContent(innerElement));
                    else if ("trim-directive-whitespaces".equals(tname))
                        trimSpaces = getTextContent(innerElement);
                    else if ("deferred-syntax-allowed-as-literal".equals(tname))
                        poundAllowed = getTextContent(innerElement);
                }
     ...
     */
    public static void makeJspPropertyGroups(Vector<JspPropertyGroup> jspProperties,
