	...
	private void doCreateHandler() throws ServletException {
		...
		if (!filters.isEmpty() && filters.get(0).getInitParams().get(WebContainerConstants.FILTER_RANKING) != null) {
			Collections.sort(filters, new Comparator<FilterModel>() {
				@Override
				public int compare(FilterModel filter1, FilterModel filter2) {
					return Integer.valueOf(filter1.getInitParams().get(WebContainerConstants.FILTER_RANKING)).compareTo(Integer.valueOf(filter2.getInitParams().get(WebContainerConstants.FILTER_RANKING)));
				}
			});
		}
