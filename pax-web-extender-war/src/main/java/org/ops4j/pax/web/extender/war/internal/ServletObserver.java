...
public class ServletObserver extends WebObserver<String> {
	...
	public ServletObserver(final WebXmlParser parser,
			WebAppPublisher publisher, WebEventDispatcher eventDispatcher,
	...
	public void addingEntries(Bundle bundle, List<String> entries) {
		...
		try {
			...
			// startup.
			if ("true".equals(opt(
					ManifestUtil.getHeader(bundle, "Webapp-Deploy"), "true"))) {
				deploy(webApp);
			} else {
