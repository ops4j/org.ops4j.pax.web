...
abstract class AbstractServerState implements ServerState
{
    ...
    private <T> T throwIllegalState(States serverState, Collection<String> supportedOperations)
    {
        throw new IllegalStateException( String.format( "server current state is: %s. The only supported operation(s): %s", serverState, supportedOperations ) );
    }
