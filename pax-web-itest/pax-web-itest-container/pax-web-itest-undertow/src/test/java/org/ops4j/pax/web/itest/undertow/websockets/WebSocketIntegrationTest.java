...
public class WebSocketIntegrationTest extends AbstractContainerTestBase {
	...
	protected WebSocketContainer getWebSocketContainer() {
		...
		try {
			Bundle undertowWebSocketClientBundle = bundle("io.undertow.websockets-jsr");
			Thread.currentThread().setContextClassLoader(undertowWebSocketClientBundle.adapt(BundleWiring.class).getClassLoader());
