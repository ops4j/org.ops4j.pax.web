...
public class ServerControllerBasicRegistrationTest extends MultiContainerTestSupport {
    ...
    public void registerFilterAndServletsUsingExcplicitBatch() throws Exception {
        ...
        // when single instance is added more than once (passed in
        // ServletModel), init(ServletConfig)
        // operates on single instance and even the Whiteboard Service
        // specification suggests using Prototype
        // Service. Otherwise, init() would be called more than once on single
        // instance providing different
        // ServletConfig objects (with different - and usually wrong
        // ServletContext)
        ...
        batch.addServletModel(server,
                new ServletModel.Builder().withServletName("my-servlet1").withUrlPatterns(new String[] { "/s/*" }) // responds
                                                                                                                   // to
                                                                                                                   // /*/s/*
                                                                                                                   // depending
                                                                                                                   // on
                                                                                                                   // context
                                                                                                                   // selector
                        .withServletReference(ref).withInitParams(initParams).withOsgiContextModel(osgiContextC) // responds
                                                                                                                 // to
                                                                                                                 // /c/s/*
                        .withOsgiContextModel(osgiContextE) // responds to
                                                            // /e/s/*
                        .withRegisteringBundle(bundle).build());
        batch.addServletModel(server,
                new ServletModel.Builder().withServletName("my-servlet2").withUrlPatterns(new String[] { "/s2/*" }) // responds
                                                                                                                    // to
                                                                                                                    // /*/s2/*
                                                                                                                    // depending
                                                                                                                    // on
                                                                                                                    // context
                                                                                                                    // selector
                        .withServletReference(ref).withInitParams(initParams).withOsgiContextModel(osgiContextC2) // responds
                                                                                                                  // to
                                                                                                                  // /c/s2/*
                        .withRegisteringBundle(bundle).build());
        ...
        // this filter is NOT registered to osgiContextC2, so should NOT be
        // mapped to /c/s2/*
        set.add(new FilterModel.Builder().withFilterName("my-filter").withUrlPatterns(new String[] { "/*" }) // maps
                                                                                                             // to
                                                                                                             // /*/*
                                                                                                             // depending
                                                                                                             // on
                                                                                                             // associated
                                                                                                             // contexts
                .withFilter(filter).withOsgiContextModel(osgiContextC) // maps
                                                                       // to
                                                                       // /c/*
        ...
        // just servlet, because /* filter doesn't use my-servlet2's
        // ServletContextHelper
        response = httpGET(port, "/c/s2/1?t=registerFilterAndServletUsingExcplicitBatch");
        System.out.println(response);
        assertTrue(response.contains("\r\nmy-servlet2[/c]"));

        // just servlet, because /* filter isn't associated with OsgiContext for
        // /e
    ...
    public void registerFilterAndServletsUsingWebContainer() throws Exception {
        ...
        Filter filter = new HttpFilter() {
            @Override
            public void init() {
                LOG.info("Filter {} initialized in {}", System.identityHashCode(this),
                        getServletContext().getContextPath());
            }

            @Override
            public void destroy() {
                LOG.info("Filter {} destroyed in {}", System.identityHashCode(this),
                        getServletContext().getContextPath());
            }
        ...
        view.registerServlet(Arrays.asList(wccC1, wccE1),
                new ServletModel.Builder().withServletName("my-servlet1").withUrlPatterns(new String[] { "/s/*" }) // responds
                                                                                                                   // to
                                                                                                                   // /c/s/*
                                                                                                                   // or
                                                                                                                   ...
                                                                                                                   // context
                                                                                                                   // selector
                        .withServletReference(ref).withInitParams(initParams).build());
        view.registerServlet(Collections.singletonList(wccC2),
                new ServletModel.Builder().withServletName("my-servlet2").withUrlPatterns(new String[] { "/s2/*" }) // responds
                                                                                                                    // to
                                                                                                                    // /c/s2/*
                                                                                                                    // depending
        ...
        // this filter is NOT registered to osgiContextC2, so should NOT be
        // mapped to /c/s2/*
        view.registerFilter(Arrays.asList(wccC1, wccD1),
                new FilterModel.Builder().withFilterName("my-filter").withUrlPatterns(new String[] { "/*" }) // maps
                                                                                                             // to
                                                                                                             // /c/*
                                                                                                             // or
                                                                                                             // /d/*
                                                                                                             ...
                                                                                                             // associated
                                                                                                             // contexts
                        .withFilter(filter).withRegisteringBundle(bundle).build());
    ...
    /**
     * <p>
     * Test for Whiteboard service registration of servlets to different OSGi
     * contexts and handling name conflicts.
     * </p>
     *
     * <p>
     * Have 3 contexts (each with single OSGi context associated):
     * <ul>
     * <li>/c1</li>
     * <li>/c2</li>
     * <li>/c3</li>
     * <li>/c4</li>
     * </ul>
     * Servlet registration plan (newer servlet has higher service.id):
     * <ul>
     * <li>"s1"(1) with rank=0 ragistered to /c1 and /c2 - should be OK</li>
     * <li>"s1"(2) with rank=3 registered to /c3 - should be OK</li>
     * <li>"s1"(3) with rank=0 registered to /c1 - should be registered as
     * disabled</li>
     * <li>"s1"(4) with rank=2 registered to /c2 and /c3 - should be registered
     * as disabled because of "s1"(2)</li>
     * <li>"s1"(5) with rank=1 registered to /c2 and /c4 - should deactivate
     * "s1"(1) from /c1 and /c2, should reactivate "s1"(3) in /c1, which was
     * previously disabled, should activate "s1"(4) in /c2 instead of "s1"(5),
     * but "s1"(4) is still shadowed in /c3 by "s1"(2), so "s1"(5) is the one
     * active in /c2</li>
     * <li>"s1"(6) with rank=0 registered to /c4 - as disabled, because shadowed
     * by "s1"(5)</li>
     * <li>"s1"(2) is unregistered from /c3 - should activate "s1"(4) in /c3 and
     * even in /c2, because in /c2 "s1"(5) is active, but with lower rank - this
     * has to change just as if "s1"(4) was newly registered. so "s1"(5) is
     * deactivated - in both /c2 and /c4, so leading to reactivation of "s1"(6)
     * in /c4</li>
     * </ul>
     * </p>
     *
    ...
    public void registerServletsConflictingByName() throws Exception {
        ...
        view.registerServlet(Arrays.asList(wcc1, wcc2),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s11).withServiceRankAndId(0, ++serviceId).build());
        ...
        view.registerServlet(Collections.singletonList(wcc3),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s12).withServiceRankAndId(3, ++serviceId).build());

        assertThat(httpGET(port, "/c1/s"), endsWith("S(1)"));
        assertThat(httpGET(port, "/c2/s"), endsWith("S(1)"));
        assertThat(httpGET(port, "/c3/s"), endsWith("S(2)"));

        // servlet#3 registered to /c1, but with higher service ID - should be
        // marked as disabled
        view.registerServlet(Collections.singletonList(wcc1),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s13).withServiceRankAndId(0, ++serviceId).build());

        assertThat(httpGET(port, "/c1/s"), endsWith("S(1)"));
        assertThat(httpGET(port, "/c2/s"), endsWith("S(1)"));
        assertThat(httpGET(port, "/c3/s"), endsWith("S(2)"));

        // servlet#4 registered to /c2 and /c3 - ranked higher than s#1 in /c2,
        // but ranked lower than s#2 in /c3
        view.registerServlet(Arrays.asList(wcc2, wcc3),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s14).withServiceRankAndId(2, ++serviceId).build());
        ...
        view.registerServlet(Arrays.asList(wcc2, wcc4),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s15).withServiceRankAndId(1, ++serviceId).build());
        ...
        // servlet#6 registered to /c4 - ranked lower than s#5 in /c4, so added
        // as disabled
        view.registerServlet(Collections.singletonList(wcc4),
                new ServletModel.Builder().withServletName("s1").withUrlPatterns(new String[] { "/s" })
                        .withServletReference(s16).withServiceRankAndId(0, ++serviceId).build());
        ...
        // activated in /c2 because s#5 in /c2 is ranked
        // lower than s#4, so s#5 disabled in /c4, so s#6 enabled in /c4
        view.unregisterServlet(new ServletModel.Builder().withServletReference(s12).withOsgiContextModel(cm3).build());
