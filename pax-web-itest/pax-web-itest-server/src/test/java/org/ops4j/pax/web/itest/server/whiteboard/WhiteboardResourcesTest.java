...
public class WhiteboardResourcesTest extends MultiContainerTestSupport {
	...
	public void twoWaysToRegisterResourcesInCustomContext() throws Exception {
		...
		properties.put(HttpWhiteboardConstants.HTTP_WHITEBOARD_RESOURCE_PATTERN, new String[] { "/files/*", "/res/*" });
		properties.put(HttpWhiteboardConstants.HTTP_WHITEBOARD_RESOURCE_PREFIX, "/resources");
		properties.put(HttpWhiteboardConstants.HTTP_WHITEBOARD_CONTEXT_SELECT, String.format("(%s=c1)",
				HttpWhiteboardConstants.HTTP_WHITEBOARD_CONTEXT_NAME));
		ServiceReference<Object> resourcesRef = mockReference(sample1, Object.class, properties, Object::new, 0L, 0);
		ServletModel model = getResourceCustomizer().addingService(resourcesRef);
		assertThat(httpGET(port, "/files/file.txt"), startsWith("HTTP/1.1 404"));
		assertThat(httpGET(port, "/res/file.txt"), startsWith("HTTP/1.1 404"));
		assertThat(httpGET(port, "/c/files/file.txt"), endsWith("hello1"));
		assertThat(httpGET(port, "/c/res/file.txt"), endsWith("hello1"));
		assertThat(httpGET(port, "/c/files/other.txt"), startsWith("HTTP/1.1 404"));

		getResourceCustomizer().removedService(resourcesRef, model);
		assertThat(httpGET(port, "/c/files/file.txt"), startsWith("HTTP/1.1 404"));
		assertThat(httpGET(port, "/c/files/other.txt"), startsWith("HTTP/1.1 404"));
		...
		assertThat(httpGET(port, "/c/resources/other.txt"), startsWith("HTTP/1.1 404"));
